...
Divide Register by Register: rx/ry
rx, rx+1 <- c(rx)/ c(ry)
rx must be 0 or 2
...
//				ryValue must be 0 or 2
				...
				if((rxValue ==0 || rxValue ==2) && (ryValue ==0 || ryValue ==2) ) {
					...
					if(UnitConverter.binaryStringToInteger(register.getGPRj(registerY)) ==0) {
						...
						//calculte quotient
						int quotient = (int) UnitConverter.binaryStringToInteger(register.getGPRj(registerX)) /
						...
						register.setGPRj(registerX, UnitConverter.integerTo16BitBinary(quotient));
